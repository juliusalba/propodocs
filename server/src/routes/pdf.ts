...
// Marine PDF Generation
router.post('/generate-marine-pdf', async (req, res) => {
    console.log('Marine PDF generation request received');
    let browser;
    try {
        const { clientName, selectedTier, addOns, totals, coverPhotoUrl } = req.body;
        console.log('Request data:', { clientName, selectedTier });

        const htmlContent = generateMarinePDFHTML(clientName, selectedTier, addOns, totals, coverPhotoUrl);
...
function generateMarinePDFHTML(clientName: string, selectedTier: string, addOns: any, totals: any, coverPhotoUrl?: string) {
    const marineTiers: Record<string, any> = {
        wake: {
            name: 'WAKE', monthly: 5000, mediaIncluded: 2500, setup: 1500,
            ...
            deliverables: [
                ...
                'Speed-to-lead automation setup',
                'Monthly insights report with optimization suggestions'
            ]
        },
        harbor: {
            name: 'HARBOR', monthly: 10000, mediaIncluded: 5000, setup: 3000,
            ...
            deliverables: [
                ...
                '1 BDC huddle per month'
            ]
        },
        offshore: {
            name: 'OFFSHORE', monthly: 18000, mediaIncluded: 9000, setup: 5000,
            ...
            deliverables: [
                ...
                '10–12 ad-creative sets per month',
                '1–2 CRO experiments per month',
                '2 BDC huddles per month + call library'
            ]
        }
    };
    ...
    if (selectedTier && marineTiers[selectedTier]) {
        ...
        const deliverablesHTML = tier.deliverables.map((d: string) => `<li>${d}</li>`).join('');
        tierHTML = `
            <div style="background: #f0f9ff; padding: 20px; border-radius: 12px; margin-bottom: 16px; border-left: 4px solid #0ea5e9;">
                <div style="font-weight: 700; color: #1f2937; margin-bottom: 8px; font-size: 18px;">${tier.name} - ${tier.description}</div>
                <div style="display: flex; justify-content: space-between; margin-bottom: 12px; padding: 12px; background: white; border-radius: 8px;">
                    <div><div style="color: #6b7280; font-size: 13px;">Monthly Investment</div><div style="font-weight: 700; color: #0ea5e9; font-size: 20px;">$${tier.monthly.toLocaleString()}</div></div>
                    <div><div style="color: #6b7280; font-size: 13px;">Media Included</div><div style="font-weight: 700; color: #10b981; font-size: 20px;">$${tier.mediaIncluded.toLocaleString()}</div></div>
                    <div><div style="color: #6b7280; font-size: 13px;">Setup Fee</div><div style="font-weight: 700; color: #6b7280; font-size: 20px;">$${tier.setup.toLocaleString()}</div></div>
                </div>
                <div style="font-weight: 600; color: #1f2937; margin-bottom: 8px; font-size: 15px;">Included Deliverables:</div>
                <ul style="margin: 0; padding-left: 20px; color: #374151; font-size: 14px; line-height: 1.8;">${deliverablesHTML}</ul>
            </div>
        `;
    }

    // Generate Scope
    const scope = generateMarineScope(selectedTier);
    const scopeHTML = `
        <div style="display: flex; gap: 20px;">
            <div style="flex: 1; background: #f0fdf4; padding: 16px; border-radius: 8px;">
                <div style="font-weight: 700; color: #166534; margin-bottom: 8px; font-size: 15px;">Included</div>
        ...
        <head>
            ...
            <style>
                * { margin: 0; padding: 0; box-sizing: border-box; }
                ...
                .logo { font-size: 32px; font-weight: 300; letter-spacing: 3px; margin-bottom: 8px; }
                .tagline { font-size: 14px; opacity: 0.9; font-weight: 300; }
                .subtitle { font-size: 16px; margin-top: 12px; font-weight: 400; opacity: 0.95; }
                .client-name { background: rgba(255, 255, 255, 0.2); padding: 12px 24px; border-radius: 8px; margin-top: 20px; display: inline-block; font-size: 18px; font-weight: 400; }
                .content { padding: 0 40px; }
                .section { margin-bottom: 32px; }
                .section-title { font-size: 20px; font-weight: 700; color: #1f2937; margin-bottom: 16px; padding-bottom: 8px; border-bottom: 2px solid #e5e7eb; }
        ...
        <body>
            ...
            <div class="content">
                ...
                <div class="section">
                    <div class="section-title">Contract Terms</div>
                    <div style="background: #f8fafc; padding: 16px; border-radius: 8px; border-left: 4px solid #6366f1;">
                        <div style="font-weight: 600; color: #1f2937; font-size: 16px; margin-bottom: 8px;">6-Month Contract (Standard Terms)</div>
...
function generateMarineScope(selectedTier: string) {
    const inclusions: string[] = [
